"""
STRATEGY 4: NEWS REVERSION
Buy the dip after sudden price spikes (overreaction to news)
"""
import json
import numpy as np

print("[*] NEWS REVERSION BACKTEST STARTING...")
...
def run_backtest():
    results = {
        'strategy': 'News Reversion',
        'description': 'Buy after sudden price drops (assuming overreaction to news)',
        'trades': [],
        'total_markets': 0,
        'trades_executed': 0,
        'wins': 0,
    ...
    spike_threshold = 0.15  # 15% price move in one period = spike
    ...
    for market in data:
        ...
        # Look for sudden drops (potential overreaction)
        for i in range(3, len(prices) - 5):
            prev_price = prices[i-1]
            curr_price = prices[i]
            
            # Detect significant drop
            if prev_price > 0.2:  # Avoid division issues
                price_change = (curr_price - prev_price) / prev_price
                
                if price_change < -spike_threshold:  # Big drop
                    entry_price = curr_price
                    
                    # Skip if already at extremes
                    if entry_price > 0.85 or entry_price < 0.1:
                        continue
                    
                    # Buy YES after drop (betting on reversion)
...
with open('backtest-results/news_reversion_results.json', 'w') as f:
    json.dump(results, f, indent=2)

print(f"[OK] NEWS REVERSION COMPLETE")
