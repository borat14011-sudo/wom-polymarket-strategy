...
def test_api_connection():
    ...
    try:
        ...
        print(f"✅ API credentials obtained: {creds[:50]}...")
        ...
        if gamma_resp.status_code == 200:
            events = gamma_resp.json().get('events', [])
            if events and events[0].get('markets'):
                ...
                try:
                    ...
                    print(f"⚠️  Could not get order book (may need auth): {e}")
        else:
            print("⚠️  Could not fetch market ID from Gamma API")
