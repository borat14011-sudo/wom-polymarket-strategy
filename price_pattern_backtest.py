#!/usr/bin/env python3
"""
REAL BACKTESTER #3: Price-Based Pattern Analysis
...
if markets:
    ...
    print(f"Price history entries: {len(sample.get('price_history', []))}")
    if sample.get('price_history'):
        print(f"Sample price entry: {sample['price_history'][0]}")
...
def analyze_price_patterns(markets):
    ...
    for market in markets:
        processed += 1
        if processed % 1000 == 0:
            print(f"Processed {processed}/{len(markets)} markets...")
    ...
    print(f"\nProcessed {processed} markets, skipped {skipped} (insufficient data)")
    return results
...
# Run the analysis
print("\n" + "="*60)
print("BACKTESTING PRICE-BASED PATTERNS")
print("="*60)
print(f"Transaction costs: {TOTAL_COST*100:.1f}% per round trip")
print(f"  - Fees: {FEE_RATE*100:.1f}%")
...
for strategy_name, trades in pattern_results.items():
    ...
    if stats:
        ...
        print(f"Median PnL: {stats['median_pnl']*100:.2f}%")
        print(f"Total PnL: {stats['total_pnl']*100:.1f}%")
        print(f"Expected Value: {stats['avg_pnl']*100:.2f}% per trade")
        print(f"Best Trade: +{stats['best_trade']*100:.1f}%")
        print(f"Worst Trade: {stats['worst_trade']*100:.1f}%")
        print(f"Std Dev: {stats['stddev']*100:.1f}%")
